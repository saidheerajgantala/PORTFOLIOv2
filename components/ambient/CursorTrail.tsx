    ...
    const render = () => {
      ...
      root.innerHTML = live
        .map((d) => {
          ...
          const opacity = 1 - age;
          return `<span data-id="${d.id}" style="position:absolute;left:${d.x}px;top:${d.y}px;width:6px;height:6px;border-radius:9999px;background:#C6FF3D;opacity:${opacity};transform:translate(-50%,-50%);transition:opacity 0.2s"></span>`;
        })
        .join('');
    };
