 ...
 * than snappy.
 *
 * Tick marks at each section position act as muted visual landmarks; only
 * the glow ball carries accent color so it stays the focal point on the rail.
 *
...
export function ScrollTrace({ role }: ScrollTraceProps) {
  ...
  // Reduced-motion path: snap to the nearest section tick based on scroll
  // fraction, no spring. Computed inline from progress each frame.
  const snapTop = useTransform(progress, (v) => {
    if (totalSlots <= 1) return '0%';
    const idx = Math.round(v * (totalSlots - 1));
    return `${(idx / (totalSlots - 1)) * 100}%`;
  });
  ...
  return (
    ...
    >
      ...
      {/* Tick marks at each section position — all muted so the glow ball
          is the only accent on the rail. */}
      {order.map((id, i) => {
        ...
        return (
          ...
          >
            <span className="inline-block h-1 w-1 rounded-full bg-muted/40" />
          </div>
        );
      })}

      {/* Active glow ball — only accent on the rail. */}
      <motion.div
        className="absolute left-0 right-0 flex items-center justify-center"
        style={reduce ? { top: snapTop } : { top: topPct }}
      >
        <span
